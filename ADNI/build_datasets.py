...
adni_nightingale_path = (
    main_path + "Biospecimen_Results/ADNINIGHTINGALELONG_05_24_21_11Mar2025.csv"
)
...
for outcome in ["allcausedementia", "alzheimers"]:
    ...
    for age_cutoff in [None, 65]:
        ...
        print(f"Applying age cutoff: {age_cutoff}")
        ...
        for subset_amyloid_positive in [False]:  # True,
            ...
            # Set save path based on outcome, amyloid positive subsetting, and age cutoff
            save_path = f"../../tidy_data/ADNI/{outcome}_outcome/"
            
            if subset_amyloid_positive:
                save_path += "amyloid_positive/"
                pet = pet[pet.AMYLOID_STATUS_COMPOSITE_REF == 1]
            
            if age_cutoff is not None: 
                save_path += f"age_{age_cutoff}/"
            ...
            for fold in range(5):
                # Process the fold
                train_set, val_set = process_fold(data, "id", fold_assignments, fold, age_cutoff=age_cutoff)
