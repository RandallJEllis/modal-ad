    ...
    for modality in ["proteomics", "neuroimaging", "cognitive_tests"]:
        for model in ["lgbm", "lrl1"]:
            ...
            for age_cutoff in [0, 65]:
                    ...
                    if "21003-" in f:
                        ...
                        if modality == "cognitive_tests":
                            ...
                            if f[:hyphen_idx] != "21003" and f[:hyphen_idx] != "31":
                                if int(f[:hyphen_idx]) in lookup_dict:
                                    ticks.append(
                            ...
                            if f[:hyphen_idx] in lookup_dict:
                                ticks.append(lookup_dict[int(f[:hyphen_idx])])
                            else:
                                ticks.append(f)
                        ...
                        allele_num = f[-3]
                        ticks.append(f"APOEe4, {allele_num} alleles")
                        ...
                        if modality == "neuroimaging":
                            if int(f[:underscore_idx]) in lookup_dict:
                ...
                # overwrite variable names with ticks
                df.feature = ticks

                # save a table of the feature importances
                df.to_csv(f"{filepath}/feature_importance_table.csv", index=False)

                # save html table of the top 50 
                df_top50 = df.sort_values('mean_importance', ascending=False).head(50)
                df_top50.columns = ['Feature', 'Mean Importance', 'SD Importance']
                # df_top50.to_html(f"{filepath}/feature_importance_table_top50.html", index=False)

                # Add Bootstrap classes for a clean look
                html = df_top50.to_html(
                    index=False,
                    classes=['table', 'table-striped', 'table-hover', 'table-bordered'],
                    border=0
                )

                # Wrap with Bootstrap CSS
                full_html = f"""
                <!DOCTYPE html>
                <html>
                <head>
                    ...
                    <style>
                        ...
                        caption {{ caption-side: top; font-size: 1.2em; font-weight: bold; margin-bottom: 10px; }}
                    </style>
                </head>
                <body>
                    <h2>Top 50 Feature Importances</h2>
                    {html}
                </body>
                </html>
                """

                with open(f"{filepath}/feature_importance_table_top50.html", 'w') as f:
                    f.write(full_html)
