...
folds <- 1:5
eval_times <- seq(3, 7) # Adjust as needed
...
for (fold in folds) {
  nri_res[[paste0("fold_", fold)]] <- net_reclassification_improvement(
    ...
    models_list$ptau[[paste0("fold_", fold)]],
...
nri_over_time <- ggplot(plot_data, aes(x = time, y = mean_value, color = Component, fill = Component, group = Component)) +
  ...
  labs(
    ...
    subtitle = "Demographics+Lancet vs. pTau-217",
...
ggsave("../../../figures/paper/a4_learn/nri_over_time.png", dpi = 400, width = 8, height = 8)
...
dca_all <- decision_curve_all_folds(
  ...
  model_names = c("demographics_lancet", "ptau"),
  val_df_l = val_df_l,
  folds = 1:5,
  times = seq(3, 7)
)
...
print(p)
ggsave("../../../figures/paper/a4_learn/dca_all_times.png", dpi = 400, width = 14, height = 10)
