...
library(this.path)
source(file.path(this.dir(), "..", "..", "..", "survival", "time2event", "plotting_common.R"))
...
create_additional_figures <- function(models, val_data_dict, times) {
  ...
  p5 <- ggplot(roc_data, aes(x = FPR, y = TPR, color = Model)) +
    ...
    coord_equal(xlim = c(-0.05, 1.01), ylim = c(-0.05, 1.01), expand = FALSE) +
    publication_theme
...
plot_all_roc_curves <- function(model_names, eval_times) {
  ...
  roc_plot <- ggplot(roc_summary, aes(x = FPR, y = mean_TPR, color = Model)) +
    ...
    coord_fixed(xlim = c(-0.05, 1), ylim = c(-0.05, 1), expand = FALSE) +
    theme_minimal() +
    theme(
      plot.title = element_text(face = "bold", size = 16, hjust = 0.5),
      plot.subtitle = element_text(size = 14, hjust = 0.5),
...
save_all_figures <- function(model_names, models_list, metrics_list, train_df_l, val_df_l, width, height, dpi, main_path, eval_times = NULL, filename_addon = "") {
  ...
  ##### FIGURE 1B: Year with largest AUC difference
  ...
  ##### Figure 1C: Sensitivity, Specificity, PPV, NPV
