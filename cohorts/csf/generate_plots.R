...
for (ad_outcome in c(
  ...
  main_path <- paste0(start_path, ad_outcome_path)
  ...
  # metrics_list <- metrics_list[metrics_list$time < 11, ]

  save_all_figures(
    modality = "csf", model_names, models_list, metrics_list, train_df_l, val_df_l,
...
ggsave("../../nacc/tidy_data/ad_outcome/brier_decomp.png", dpi = 400, width = 14, height = 8)
...
for (fold in folds) {
  nri_res[[paste0("fold_", fold)]] <- net_reclassification_improvement(
    models_list$demographics_lancet[[paste0("fold_", fold)]],
    models_list$csf[[paste0("fold_", fold)]],
...
nri_over_time <- ggplot(plot_data, aes(x = time, y = mean_value, color = Component, fill = Component, group = Component)) +
  ...
  labs(
    ...
    subtitle = "Demographics + Lancet vs. CSF markers",
...
ggsave("../../nacc/tidy_data/ad_outcome/nri_over_time.png", dpi = 400, width = 8, height = 8)
...
dca_all <- decision_curve_all_folds(
  ...
  model_names = c("demographics_lancet", "csf"),
  val_df_l = val_df_l,
  folds = 1:5,
  times = seq(2, 10)
)
...
print(p)
ggsave("../../nacc/tidy_data/ad_outcome/dca_all_times.png", dpi = 400, width = 14, height = 10)
...
m_list <- models_list$centiloids_demographics
...
model1 <- "demographics_lancet"
model2 <- "csf_demographics_lancet"
...
csf_demo_lancet_trocs <- pull_trocs(metrics_list, "csf_demographics_lancet")

# demo vs centiloids+demo
pvals_compare_trocs <- compare_tvaurocs(
  demo_lancet_trocs,
  csf_demo_lancet_trocs
)
pvals_compare_trocs$all_results <- pvals_compare_trocs$all_results %>% filter(time >= 2 & time <= 10)
...
ggsave(paste0(main_path, "pvalue_histogram_CSF_Demo_Lancet_vs_Demo_Lancet.pdf"),
...
# since we need coefficients for abeta, ptau, and ttau, we need to print them out separately
for (model_name in names(models_list)) {
  if ("csf" %in% model_name) {
    for (marker in c("abeta", "ptau", "tau", "ratio_ptau_abeta")) {
      print(paste0("Model: ", model_name, " Marker: ", marker))
      hazard_ratios <- c()
      pvalues <- c()
      for (fold in 1:5) {
        model <- models_list[[model_name]][[paste0("fold_", fold)]]
        hazard_ratios <- c(hazard_ratios, exp(coef(model)[marker]))
        pvalues <- c(pvalues, summary(model)$coefficients[marker, "Pr(>|z|)"])
      }
      print(paste0("Mean hazard ratio: ", mean(hazard_ratios)))
      print(paste0("SD hazard ratios: ", sd(hazard_ratios)))
      print(paste0("Range of p-values: ", range(pvalues)))
    }
  }
}
...
auc_summary <- auc_summary %>% filter(time >= 2 & time <= 10)
...
for (model_name in names(models_list)) {
  if (grepl("lancet", model_name)) {
    for (fold in 1:5) {
      ...
      gdtotal_row <- summary(model)$coefficients["gds_score", ]
