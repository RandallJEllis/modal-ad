...
calculate_brier_scores <- function(model, data, times) {
  ...
  brier_scores <- sapply(times, function(t) {
    tryCatch(
      ...
      error = function(e) {
        warning(sprintf(
          "Error calculating Brier score at time %f: %s", t,
          e$message
        ))
        return(NA)
      }
...
print_model_stats <- function(models_list, var_string, return_table = FALSE) {
  ...
  if (return_table) {
    ...
    coefs_summary <- data.frame(
      ...
      group_by(model) %>%
      summarise(
        coef = paste0(round(mean(coef), 3), " (", round(sd(coef), 3), ")"),
  ...
  for (model_name in model_names) {
    ...
    for (time in unique(model_df$Time)) {
      ...
      if (nrow(model_df_time) < 2) {
        warning(sprintf(
          "Insufficient data points for model %s at time %s",
          model_name, time
        ))
        next
      }
      ...
      pAUC <- tryCatch(
        ...
        error = function(e) {
          warning(sprintf(
            "Integration failed for model %s at time %s: %s",
  ...
  for (t in seq(3, 8)) {
    difference_dfs <- rbind(
      ...
      df_sespppvnpv %>%
        ...
        mutate(
          difference_sensitivity = mean_sensitivity - mean_sensitivity[model == model2],
...
# Function to calculate Net Reclassification Improvement (NRI) for Cox models
# Uses nricens package (continuous NRI when cut = 0)
net_reclassification_improvement <- function(model_ref, model_new, data, times) {
  # Initialize results dataframe
  results <- data.frame(
    time = numeric(),
    nri = numeric(),
    nri_event = numeric(),
    nri_nonevent = numeric(),
    stringsAsFactors = FALSE
  )

  for (t in times) {
    # Create prediction dataset up to time t
    # Using the same logic as calculate_brier_at_time to handle time-varying covariates/data structure
    pred_data <- data %>%
      filter(tstart <= t) %>%
      group_by(id) %>%
      filter(row_number() == n()) %>% # Get last record for each subject
    ...
    if (nrow(pred_data) == 0) {
      warning(paste("No data available for time", t))
      next
    }

    # Calculate risks (1 - survival probability) at time t
    risk_ref <- tryCatch(
      {
        probs <- pec::predictSurvProb(model_ref, newdata = pred_data, times = t)
        if (is.matrix(probs)) probs <- as.numeric(probs)
        1 - probs
      },
      error = function(e) {
        return(NULL)
    ...
    risk_new <- tryCatch(
      {
        probs <- pec::predictSurvProb(model_new, newdata = pred_data, times = t)
        if (is.matrix(probs)) probs <- as.numeric(probs)
        1 - probs
      },
      error = function(e) {
    ...
    if (is.null(risk_ref) || is.null(risk_new)) {
      warning(sprintf("Risk prediction failed at time %f", t))
      next
    }

    # Calculate NRI using nricens
    # cut = 0 implies continuous NRI
    # n.iter = 0 to return only point estimates (faster)
    nri_obj <- tryCatch(
      {
        nricens(
          time = pred_data$tstop,
      ...
      error = function(e) {
        warning(sprintf("nricens failed at time %f: %s", t, e$message))
        return(NULL)
      }
    )

    if (!is.null(nri_obj)) {
      # nricens returns a matrix 'nri' with rows 'NRI', 'NRI+', 'NRI-' and column 'Estimate'
      nri_val <- nri_obj$nri["NRI", "Estimate"]
      nri_event_val <- nri_obj$nri["NRI+", "Estimate"]
      nri_nonevent_val <- nri_obj$nri["NRI-", "Estimate"]

      results <- rbind(results, data.frame(
        time = t,
        nri = nri_val,
        nri_event = nri_event_val,
        nri_nonevent = nri_nonevent_val
      ))
    }
  }
...
# Function to calculate Decision Curve Analysis (DCA) for Cox models
# Calculates net benefit across a range of threshold probabilities at specified time points
decision_curve_analysis <- function(model, data, times,
  ...
  results <- data.frame(
    ...
    threshold = numeric(),
    net_benefit = numeric(),
    net_benefit_all = numeric(), # Treat all strategy
    net_benefit_none = numeric(), # Treat none strategy
    stringsAsFactors = FALSE
  )

  for (t in times) {
    # Create prediction dataset up to time t
    # Using the same logic as calculate_brier_at_time to handle time-varying covariates
    pred_data <- data %>%
      filter(tstart <= t) %>%
      group_by(id) %>%
      filter(row_number() == n()) %>% # Get last record for each subject
    ...
    # Calculate predicted risks (1 - survival probability) at time t
    pred_risk <- tryCatch(
      {
        probs <- pec::predictSurvProb(model, newdata = pred_data, times = t)
        if (is.matrix(probs)) probs <- as.numeric(probs)
        1 - probs
      },
      error = function(e) {
        warning(sprintf("Risk prediction failed at time %f: %s", t, e$message))
        return(NULL)
      }
    )

    if (is.null(pred_risk)) next

    # Determine actual outcomes at time t
    # Event occurred if tstop <= t and event == 1
    actual_outcome <- ifelse(pred_data$tstop <= t & pred_data$event == 1, 1, 0)

    # Create a temporary dataframe for dcurves
    df_dca <- data.frame(
    ...
    # Run DCA using dcurves package
    # suppress messages about calculating net benefit
    dca_result <- tryCatch(
      {
        as.data.frame(suppressMessages(dcurves::dca(
          outcome ~ pred_risk,
          data = df_dca,
          thresholds = thresholds,
          as_probability = "pred_risk"
        ))$dca)
      },
      error = function(e) {
        warning(sprintf("dca function failed at time %f: %s", t, e$message))
        return(NULL)
      }
    )
    ...
    # dcurves returns columns: variable, threshold, net_benefit, label, etc.
    # We need to pivot this to get columns for model, all, and none

    # Extract model results
    model_nb <- dca_result %>%
      filter(variable == "pred_risk") %>%
      select(threshold, net_benefit) %>%
      rename(net_benefit_model = net_benefit)

    # Extract Treat All results
    all_nb <- dca_result %>%
      filter(variable == "all") %>%
      select(threshold, net_benefit) %>%
      rename(net_benefit_all = net_benefit)

    # Extract Treat None results
    none_nb <- dca_result %>%
      filter(variable == "none") %>%
      select(threshold, net_benefit) %>%
      rename(net_benefit_none = net_benefit) # This should be 0 across

    # Join them together
    # Note: dca might return slightly different thresholds if they are calculated,
    # but since we provided them, it should be consistent.

    processed_results <- model_nb %>%
      left_join(all_nb, by = "threshold") %>%
      left_join(none_nb, by = "threshold") %>%
      mutate(
        time = t,
        net_benefit_none = ifelse(is.na(net_benefit_none), 0, net_benefit_none)
      ) %>%
      rename(net_benefit = net_benefit_model) %>%
      select(time, threshold, net_benefit, net_benefit_all, net_benefit_none)

    results <- rbind(results, processed_results)
  }

  return(results)
}

# Function to calculate DCA and compare multiple models
decision_curve_comparison <- function(models_list, model_names, data, times,
                                      thresholds = seq(0.01, 0.99, by = 0.01)) {
  # Initialize results dataframe
  all_results <- data.frame()
  ...
  for (model_name in model_names) {
    ...
    if (nrow(dca_results) > 0) {
      ...
      all_results <- rbind(all_results, dca_results)
    }
  }
...
plot_decision_curves <- function(dca_data, time_point, model_colors = NULL) {
  # Filter to specific time point
  plot_data <- dca_data %>%
    filter(time == time_point)

  if (nrow(plot_data) == 0) {
    warning(paste("No data available for time", time_point))
    return(NULL)
  }

  # Pivot to long format for model comparison
  model_data <- plot_data %>%
    select(threshold, model, net_benefit) %>%
    distinct()

  # Get reference lines (treat all/none) from first model (they're the same for all)
  ref_data <- plot_data %>%
    filter(model == unique(plot_data$model)[1]) %>%
    select(threshold, net_benefit_all, net_benefit_none) %>%
    pivot_longer(
      cols = c(net_benefit_all, net_benefit_none),
  ...
  p <- ggplot() +
    ...
    # Formatting
    coord_cartesian(ylim = c(-0.1, max(c(
      model_data$net_benefit,
      ref_data$net_benefit
    ), na.rm = TRUE) * 1.1)) +
    labs(
      title = paste("Decision Curve Analysis at Year", time_point),
  ...
  if (!is.null(model_colors)) {
    p <- p + scale_color_manual(values = model_colors)
  }

  return(p)
}

# Function to calculate DCA across all folds and average results
decision_curve_all_folds <- function(models_list, model_names, val_df_l, folds = 1:5,
                                     times, thresholds = seq(0.01, 0.99, by = 0.01)) {
  # Initialize results dataframe
  all_results <- data.frame()
  ...
  for (fold in folds) {
    ...
    for (model_name in model_names) {
      ...
      data <- val_df_l[[fold_name]]

      dca_results <- tryCatch(
        {
        ...
        error = function(e) {
          warning(sprintf(
            "DCA failed for model %s, fold %d: %s",
            model_name, fold, e$message
          ))
          return(NULL)
        }
      )

      if (!is.null(dca_results) && nrow(dca_results) > 0) {
        dca_results$model <- model_name
        dca_results$fold <- fold
        all_results <- rbind(all_results, dca_results)
      }
    }
  }
...
# Function to summarize DCA results across folds
summarize_dca_across_folds <- function(dca_all_folds) {
  summary_df <- dca_all_folds %>%
    group_by(time, threshold, model) %>%
    summarise(
      mean_net_benefit = mean(net_benefit, na.rm = TRUE),
      sd_net_benefit = sd(net_benefit, na.rm = TRUE),
      se_net_benefit = sd_net_benefit / sqrt(n()),
      mean_nb_all = mean(net_benefit_all, na.rm = TRUE),
      sd_nb_all = sd(net_benefit_all, na.rm = TRUE),
      se_nb_all = sd_nb_all / sqrt(n()),
      .groups = "drop"
    )
...
# Function to plot decision curves for all time points (faceted)
data_decision_curves_all_times <- function(dca_all_folds, model_colors = NULL,
                                           model_labels = NULL) {
  # Summarize across folds
  summary_df <- summarize_dca_across_folds(dca_all_folds)

  # Get reference lines (treat all/none) - same for all models
  ref_data <- summary_df %>%
    filter(model == unique(summary_df$model)[1]) %>%
    select(time, threshold, mean_nb_all) %>%
    mutate(strategy = "Treat All") %>%
    rename(net_benefit = mean_nb_all)

  # Add treat none (always 0)
  ref_none <- ref_data %>%
    mutate(strategy = "Treat None", net_benefit = 0)

  ref_data <- rbind(ref_data, ref_none)

  # Apply model labels - use get_model_labels if not provided
  if (is.null(model_labels)) {
    model_labels <- get_model_labels(unique(summary_df$model))
  }
  ...
  return(list(ref_data = ref_data, summary_df = summary_df))
}
