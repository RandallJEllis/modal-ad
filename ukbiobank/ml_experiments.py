...
def parse_args():
    ...
    parser.add_argument(
        "--model", type=str, default="lgbm", help="options: lgbm, lrl1"
    )
...
def get_time_budget(experiment, data_modality, model, age_cutoff):
    ...
    Returns:
        int or None: The time budget in seconds.
    """
...
def save_results(
    ...
    """
    Save the results of a fitted classifier to CSV files.
    Parameters:
    directory_path (str): The directory where the results will be saved.
    estimator (object): The fitted model object.
    ...
    2. Creates a pandas Series with the model name and configuration.
    ...
    if model_name is None:
        model_name = estimator.best_estimator
    if hyperparams is None:
        hyperparams = estimator.best_config

    series_model = pd.Series(
        [region_index, region, model_name, hyperparams],
    ...
    train_res, threshold = ml_utils.calc_results(y_train, train_probas, beta=1)
    train_res = pd.concat([series_model, train_res])
    train_labels_l.append(y_train)
    train_probas_l.append(train_probas)
    ...
    test_res = pd.concat([series_model, test_res])
    test_labels_l.append(y_test)
    test_probas_l.append(test_probas)
